...
public:
  ...
  X Dfs(int v, int p, int f) {
    if(dp[v][f] >= 0) return dp[v][f];
    vl a, b;
    for(auto next: node[v].edge) {
      int w = next.to;
      if(w == p) continue;
      a.push_back(Dfs(w, v, 1) + next.cost);
      b.push_back(Dfs(w, v, 0));
    }
    vector<pair<ll,pl>> pp(sz(a));
    rep(i,sz(a)) {
      pp[i] = {a[i] - b[i], {a[i], b[i]}};
    }
    sort(pp.begin(), pp.end(), [](pair<ll,pl> a, pair<ll,pl> b){
      return a.first > b.first;
    });
    ll res = 0;
    rep(i,sz(a)) {
      if(i < k - f && pp[i].first > 0) {
	res += pp[i].second.first;
      }
      else {
	res += pp[i].second.second;
      }
    }
    return dp[v][f] = res;
  }
