...
const int NMAX=100000; // we can calculate nCk until n is equal to NMAX
...
int main()
{
  ...
  vector<vector<mint>> k(n+1, vector<mint>(m+1));
  rep(i,m+1) k[0][i].x = 1;
  rep1(i,n) {
    rep1(j,m) {
      if(i > j) k[i][j].x = 0;
  ...
  rep(i,n+1) dp[i][0].x = i;
  
  rep1(i,n) {
    rep1(j,m) {
      dp[i][j] = dp[i-1][j] + Comb(i-1+j, j) + dp[i][j-1] - ( Comb(i-1+j, j-1) - k[i][j-1]);
    }
  }
