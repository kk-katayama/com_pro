...
public:
  ...
  void Solve() {
	...
	int w = next.to;
	if (w == par[i]) continue;
	if(chmax(kata[i], size[w])) {
	  idx[i] = w;
	}
	sum += size[w];
      }
      if(chmax(kata[i], n - 1 - sum)) {
	idx[i] = par[i];
      }
    }
    ...
    if(f) {
      cout << 1 << " " << node[0].edge[0].to+1 << "\n";
      cout << 1 << " " << node[0].edge[0].to+1 << "\n";
    }
    else {
      for(auto next: node[id].edge) {
	int w = next.to;
	if(w != id2) {
	  cout << min(id,w)+1 << " " << max(id,w)+1 << "\n";
	  cout << min(id2,w)+1 << " " << max(id2,w)+1 << "\n";
	  break;
	}
      }
    }
