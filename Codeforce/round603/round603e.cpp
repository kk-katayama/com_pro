...
int main()
{
  auto f = [&](pl a, pl b)->pl{ return {max(a.first,b.first), min(a.second, b.second)}; };
  auto h = [&](ll a, ll b){ return a + b; };
  auto g = [&](pl a, ll b)->pl{ return {a.first + b, a.second + b}; };
  auto p = [&](ll a, int b){ return a; };  
  pl id1 = {-1, 0};
  ll id2 = 0;
  LST<decltype(f), decltype(h), decltype(g), decltype(p), pl, ll> lst(f, h, g, p, id1, id2);
  ...
  rep(i,n) {
    if(s[i] == 'R') {
      ...
      lst.update(cur, n+1, 0 - v[cur]);
      sum += 0 - v[cur];      
      v[cur] = 0;
    }

    pl tmp = lst.query(0, n+1);
    if(sum == 0 && tmp.second >= 0) {
      cout << tmp.first << " ";
    }
    else {
      cout << -1 << " ";
