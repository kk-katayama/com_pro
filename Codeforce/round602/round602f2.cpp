...
vector<ll> simple_convolve(vector<ll> &a,const vector<ll> &b){
  vector<mint> aa,bb;
  rep(i,sz(a)) {
    aa.push_back(mint(a[i]));
  }
  rep(i,sz(b)) {
    bb.push_back(mint(b[i]));
  }      
  ...
  aa.resize(t);
  bb.resize(t);
  vector<mint> A = ntt(aa);
  vector<mint> B = ntt(bb);
  rep(i,t){
    A[i] *= B[i];
  }
  A = ntt(A,true);
...
int main()
{
  ...
  while(cnt > 0) {
    if(cnt & 1) {
      res = naive_convolve<mint>(res, tmp);
    }
    cnt >>= 1;
    if(cnt == 0) break;
    tmp = naive_convolve<mint>(tmp, tmp);
  }
