...
int main()
{
  ...
  vl path(m, 0);
  rep(i,m) {
    vi tmp = tr.get_pass(u[i], v[i]);
    rep(j,sz(tmp)-1) {
      ll id = mp[{min(tmp[j], tmp[j+1]), max(tmp[j], tmp[j+1])}];
      path[i] |= (1LL << id);
    }
  }
  ...
  vector<ll> pow2(110);
  pow2[0] = 1;
  rep(i,100) pow2[i+1] = pow2[i]*2;

  ll res = 0;
  rep(S, (1 << m)) {
    ll mask = 0;
    rep(i,m) {
      if((S >> i) & 1) mask |= path[i];
    }
    if(__builtin_popcount(S) & 1) {
      res += pow2[n-1] - pow2[n-1-__builtin_popcountll(mask)];
    }
    else {
      res -= pow2[n-1] - pow2[n-1-__builtin_popcountll(mask)];      
    }
