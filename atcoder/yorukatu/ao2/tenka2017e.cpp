...
public:
  ...
  void build2() {
    ...
    sort(vals.begin(), vals.end(), [](T a, T b) {
      if(a * b > 0) return a > b;
      else return a < b;
    });
    vals.erase(unique(vals.begin(), vals.end()), vals.end());
...
int main()
{
  ...
  rep(dammy, 50) {
    ...
    int cnt = 0;
    rep(i,n) {
      cnt += seg.query(0, cp.data[p[i].second]);
      seg.add(cp.data[p[i].second], 1);
    }
    if(cnt >= (n*(n-1)/2 + 1) / 2) ub = mid;
    else lb = mid;
  }
  ld resx = ub;
  ...
  rep(dammy, 50) {
    ...
    int cnt = 0;
    rep(i,n) {
      cnt += seg.query(0, cp2.data[p[i].second]);
      seg.add(cp2.data[p[i].second], 1);
    }
    if(cnt >= (n*(n-1)/2 + 1) / 2) ub = mid;
    else lb = mid;
  }
