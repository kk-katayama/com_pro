...
vector<complex<ld>> fft(vector<complex<ld>> a,bool inverse = false){
  ...
  for(int b=1;b<n;b*=2){
    rep(j,b){
      complex<ld> w = polar(1.0,(inverse?-1:1)*(2*M_PI)/(2*b)*j);
      for(int k=0;k<n;k+=b*2){
	complex<ld> s = a[j+k];
...
vector<complex<ld>> fft(vector<ld> a,bool inverse = false){
  ...
  rep(i,a.size()) a_complex[i] = complex<ld>(a[i],0);
...
int main()
{
  ...
  //  vector<ld> conv = convolve(cnt, cnt);
  vl conv = convolve(cnt, cnt, (ll)50000000000);
  //  vl conv = convolve(cnt, cnt, (ll)67280421310721);
  ll res = 0;
  rep(i,sz(conv)) {
    res += conv[i]* pow2[i%(P-1)];
  }
