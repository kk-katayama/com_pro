...
//**********2円の共通部分の面積**********
ld IntersectionCircle(Circle &A, Circle &B) {
  int flag = CrossCheckCircle(A, B);
  if(flag == 3 || flag == 4) return 0; // 共通部分なし
  else if(flag == 0 || flag == 1) { // 内包
    return min(A.Area(), B.Area());
  }
  else {
    vector<Vec2> cp = CrossPointCircleCircle(A, B);
    ld res = 0;
    ld Acos = Cos(Line(A.p, B.p), Line(A.p, cp[0]));
    ld Bcos = Cos(Line(B.p, A.p), Line(B.p, cp[0]));
    ld cA = (Acos > 0 ? 1 : -1);
    ld cB = (Bcos > 0 ? 1 : -1);
    ld HMA = A.Area() * acos(Acos) / PI - cA * abs(Area(A.p, cp[0], cp[1]));
    ld HMB = B.Area() * acos(Bcos) / PI - cB * abs(Area(B.p, cp[0], cp[1]));
    return HMA + HMB;
  }
}
